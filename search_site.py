...
def main(args):
    ...
    aa_code = atom_info.letter_to_aa(args.resid[0])
    background_path = 'data/background_stats/{}_background.pkl'.format(aa_code)
    ...
    for it in range(args.num_iter):
        ...
        if args.verbose:
            iteration = it + 1
            len_new = len(new)
            print("Iteration " + str(iteration) + ": " + str(len_new) + " new results")
...
if __name__ == '__main__':
    ...
    if not args.outfile:
        ...
        args.outfile = '{}_{}_{}_search.csv'.format(pdb_from_fname(args.pdb), args.chain, args.resid)
